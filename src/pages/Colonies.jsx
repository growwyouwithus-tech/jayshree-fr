...
const Colonies = () => {
  ...
  }, [dispatch, isAuthenticated]) // Re-fetch when auth changes

  // Debug logging
  useEffect(() => {
    console.log('🏘️ Colonies Page: Data updated:', {
      count: colonies.length,
      isAuthenticated,
      colonies: colonies.map(c => ({ 
        name: c.name, 
        images: c.mapImages?.length || 0,
        price: c.basePricePerGaj 
      }))
    })
            ...
            {filteredColonies.map((colony) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={colony._id}>
                <Card
                  ...
                  onClick={() => navigate(`/colonies/${colony._id}`)}
                >
                  <CardMedia
                    component="img"
                    height="180"
